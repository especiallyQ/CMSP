<template>
    <div class="template-form-wrapper">
        <el-form
            ...
            label-width="60px">
            <el-form-item
                :label="$t('chainNode.nodeOrg')"
                prop="nodeOrg">
                <div class="form-div__unselect">{{ formData.nodeOrg }}</div>
            </el-form-item>
            <el-form-item
...
export default {
    ...
    methods: {
        modelClose() {
            this.$emit('close', String(this.dialogOptions.operateType));
        },
        // 获取页面初始化数据
        fetchInitData() {
            getBacklogDetail4JoinInNode({id: this.dialogOptions.id}).then(res => {
                if (res.data.code === 0) {
                    let resData = res.data.data || {};
                    this.formData.nodeOrg = resData.orgName;
